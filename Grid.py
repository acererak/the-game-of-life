#
# Conway's Game of Life: cellular automata and image processing
#
# See life.py and image.py for examples that use the
# code below.
#
...
""" Grid class
    ...
    The value 'v' stands for a cell's state. The object 'vs' holds
    ...
    greyscale image.  Low values code for darker pixels, higher
    values code for whiter pixels.  The rule, then, acts in a
    similar way to provide an image processing filter.

    Examples of image processing rules can be found in image.py.
"""
...
class Grid(Frame):
  ...
  def get(self,row,column):
    """ Accesses the state of the cell at the given
        ...
        NOTE: row and column are interpreter modulo
  ...
  def squareAt(self,row,column):
      ...
      return (8+column*self.size, 8+row*self.size, \
              8+(column+1)*self.size, 8+(row+1)*self.size)
  ...
  def advance(self):
      ...
      if self.generations == None or self.clock < self.generations: 
          ...
          for r in range(self.rows):
              for c in range(self.columns):
                  self.set(r,c,self.rule(self.get(r,c),self.around(r,c)))
  ...
  def keypress(self,event):
      ...
      # RETURN turns simulation on/off
  ...
  def tick(self):
      ...
      # if we're still running, advance
      if self.running:
          self.advance()
